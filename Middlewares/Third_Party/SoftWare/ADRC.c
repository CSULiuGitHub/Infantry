...
Fhan_Data ADRC_Pitch_Speed_Controller;
...
void ADRC_Init(Fhan_Data *fhan_Input, float *ADRC_Unit)
{
  fhan_Input->r=ADRC_Unit[0];
  fhan_Input->h=ADRC_Unit[1];
  fhan_Input->N0=(uint16_t)(ADRC_Unit[2]);
...
void Fhan_ADRC(Fhan_Data *fhan_Input,float expect_ADRC)//安排ADRC过度过程
{
  ...
  a1=Quake_Sqrt(d*(d+8*Misc_Fabsf(y)));//a1=sqrt(d*(d+8*ABS(y))])
...
float Fal_ADRC(float e,float alpha,float zeta)
{
    ...
    fal_output=e*s/(powf(zeta,1-alpha))+powf(Misc_Fabsf(e),alpha)*Sign_ADRC(e)*(1-s);
    return fal_output;
}
...
void ADRC_Control(Fhan_Data *fhan_Input,float expect_ADRC,float feedback_ADRC)
{
     ...
     */
      ...
      fhan_Input->u=Constrain_Float(fhan_Input->u0,-30000,30000);
}
